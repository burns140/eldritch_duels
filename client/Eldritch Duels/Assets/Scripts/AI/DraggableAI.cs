...
public class DraggableAI : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
	public AIDuel duelScript; // to access duelscript functions
	...
	void Start(){
		duelScript = GameObject.Find("DuelScriptObject").GetComponent<AIDuel>();
    ...
    public void OnBeginDrag(PointerEventData eventData) {
		if(duelScript.currentPhase == Phase.MAIN && duelScript.myTurn){
    ...
    public void OnEndDrag(PointerEventData eventData) {
		if(duelScript.currentPhase == Phase.MAIN && duelScript.myTurn){
			...
			if(startArea.name == "HandAreaPanel" && this.transform.parent.name.Equals("MyPlayAreaPanel") && duelScript.CanCast(this.gameObject)){
				...
				Debug.Log("casting card:"+this.name);
				duelScript.CastCard(this.name, this.gameObject);
			}
